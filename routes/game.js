...
// @access  User role
router.get("/select", isLoggedIn, async (req, res, next) => {
...
// @access  User role
router.get("/:quizId", isLoggedIn, async (req, res, next) => {
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (num_questions <= current_question) {
        ...
        //update user's cookie with new points
